...
def balanced_category_sampling(files,
    ...
    annot_df = (annot_df[annot_df.category_id.isin(requested_cats)]
                .sort_values(by='area', ascending=False)
                .drop_duplicates(subset='image_id'))

    annot_df_out = (annot_df.groupby('category_id')
                            .apply(lambda x: x[:images_per_class[x.category_id.values[0]]])
                            .reset_index(drop=True)
                            )

    if len(annot_df_out) < size:
        dif = size - len(annot_df_out)
        annot_df_out = pd.concat([annot_df_out, annot_df[-annot_df.image_id.isin(annot_df_out.image_id.to_list())].sample(dif)])


    image_id_distributed = annot_df_out.image_id.to_list()
    ...
    return images[(images.id.isin(image_id_distributed))], annot_df_out
